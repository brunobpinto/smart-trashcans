...
export function StatusesDeleteDialog({
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const message = data?.error ?? "Failed to delete status."
        setError(message)
        showErrorToast({
          title: "Delete failed",
          description: message,
      ...
      const message = "Unexpected error deleting status."
      setError(message)
      showErrorToast({
        title: "Delete failed",
        description: message,
  ...
  const descriptionText = trashcanName
    ? `Are you sure you want to delete this status for trashcan "${trashcanName}"? This action cannot be undone.`
