...
interface DashboardUsageChartProps {
  dailyData: DashboardUsagePoint[]
  hourlyTodayData: DashboardUsagePoint[]
}
    ...
    <Card className="mt-4 ml-4 -mr-4">
      <CardHeader className="flex flex-row items-center justify-between gap-3">
        <div>
          ...
          <CardDescription>
            Total number of uses per day across all trashcans.
          </CardDescription>
        </div>
      ...
      <CardContent className="pt-2">
        <ChartContainer config={chartConfig} className="h-40 w-full">
          <BarChart accessibilityLayer data={filteredData}>
            ...
            <ChartTooltip
              cursor={false}
              content={<ChartTooltipContent hideLabel className="w-36" />}
            />
