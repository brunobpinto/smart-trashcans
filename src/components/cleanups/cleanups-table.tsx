...
import { CleanupsEditDialog } from "./cleanups-edit"
import { CleanupsDeleteDialog } from "./cleanups-delete"
import { CleanupsCreateDialog } from "./cleanups-create"
...
export function CleanupsTable({
  columns,
  data,
}: DataTableProps<CleanupRow, unknown>) {
  ...
  return (
    <div className="mt-4 flex w-full flex-col gap-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <input
          type="text"
          placeholder="Search by trashcan, employee or location..."
          value={searchQuery}
          onChange={(event) => {
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-40">
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    showInfoToast({
                                      title: "ID Copied",
                                      description:
                                        "Cleanup ID was copied to the clipboard.",
                                    })
                                  } catch (error) {
                                    ...
                                    showErrorToast({
                                      title: "Copy Failed",
                                      description:
                                        "Could not copy the cleanup ID to the clipboard.",
                                    })
                                  }
                                }}
                              >
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  router.push(
                                    `/employees/${row.original.workerId}`
                                  )
                                }
                              >
                                View Employee
                              ...
                              <CleanupsEditDialog cleanup={row.original} />
                              <DropdownMenuSeparator />
                              <CleanupsDeleteDialog
                                id={row.original.id}
